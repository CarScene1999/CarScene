...
export class ObjectStorageService {
  ...
  async downloadPublicObject(file: File, res: Response, cacheTtlSec: number = 3600) {
    try {
      const [metadata] = await file.getMetadata();
  ...
  async getObjectEntityFile(objectPath: string): Promise<File> {
    ...
    } else if (objectPath.startsWith("/public-objects/")) {
      const parts = objectPath.slice(1).split("/");
      if (parts.length < 2) {
        throw new ObjectNotFoundError();
      }
      entityId = parts.slice(1).join("/");
      entityDir = this.getPublicObjectDir();
    } else {
      throw new ObjectNotFoundError();
    }
  ...
  normalizeObjectEntityPath(rawPath: string): string {
    ...
    // Remove leading slash from privateDir for comparison
    ...
    let publicDir = this.getPublicObjectDir();
    // Remove leading slash from publicDir for comparison
    ...
    if (rawObjectPath.startsWith(publicDir)) {
      const entityId = rawObjectPath.slice(publicDir.length);
      return `/public-objects/${entityId}`;
    }
  ...
  async trySetObjectEntityAclPolicy(
    ...
    // For public objects, skip ACL setting - they're accessible by design
